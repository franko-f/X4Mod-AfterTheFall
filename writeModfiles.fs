...
let factionToDLCMap =
    Map [
        // Note: This is a list of 2 element tuples! fantomas removes
        // the explicit ';' I had at the end to make this clear.
        "argon", "core"
        "hatikvah", "core"
        "antigone", "core"
        "teladi", "core"
...
let dlcFactions dlc =
    factionToDLCMap |> Map.filter (fun k v -> v = dlc) |> Map.keys |> List.ofSeq
...
let copyFileToDlc dlc (filename: string) =
    ...
    System.IO.File.Copy(__SOURCE_DIRECTORY__ + "/mod_xml/" + filename, fullname, true)
    |> ignore
